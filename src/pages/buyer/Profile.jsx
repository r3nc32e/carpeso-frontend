...
import { Eye, EyeOff, Check, Pencil, X, MapPin, Plus, Trash2 } from 'lucide-react';
...
function Profile() {
    ...
    const [newAddress, setNewAddress] = useState({ label: '', cityName: '', barangayName: '', streetNo: '' });
    ...
    const handleNewAddressCityChange = async (cityId) => {
        ...
        setNewAddress(prev => ({ ...prev, cityName: city?.name || '', barangayName: '' }));
        if (cityId) {
    ...
    const handleAddAddress = async () => {
        ...
        try {
            ...
            setNewAddress({ label: '', cityName: '', barangayName: '', streetNo: '' });
    ...
    const handleRequestPasswordOtp = async () => {
        if (!passwordForm.currentPassword) { setError('Please enter your current password first!'); return; }
        if (passwordForm.newPassword !== passwordForm.confirmPassword) { setError('New passwords do not match!'); return; }
        if (passwordForm.newPassword.length < 8) { setError('Password must be at least 8 characters!'); return; }
        setSendingOtp(true);
        setError('');
        try {
    ...
    const handleChangePasswordWithOtp = async () => {
        if (!passwordOtp || passwordOtp.length < 6) { setError('Please enter the OTP!'); return; }
        setLoading(true);
        setError('');
        try {
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                        ...
                        <span className="text-xs px-2 py-0.5 bg-red-100 text-red-600 rounded-full font-semibold uppercase">{user?.role}</span>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6">
                    ...
                    <div className="space-y-3">
                        {[
                            ...
                            { label: 'Account Status', value: profileData.active ? '✅ Active' : '❌ Inactive' },
                            { label: 'Member Since', value: profileData.createdAt ? new Date(profileData.createdAt).toLocaleDateString('en-PH') : '—' },
                    ...
                    <div className="mt-4 bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl text-xs">
                        ℹ️ To update personal info, use the Edit button on your profile card above.
                    </div>
                </div>
            )}
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        {savedAddresses.length < 5 && (
                            <button onClick={() => setShowAddAddressForm(!showAddAddressForm)}
                    ...
                    {savedAddresses.map(addr => (
                        <div key={addr.id} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl border border-gray-200">
                            ...
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    {addr.default && (
                                        <span className="text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-bold">Default</span>
                                    )}
                                </div>
                    ...
                    {showAddAddressForm && (
                        <div className="border border-gray-200 rounded-xl p-4 space-y-3">
                            <div className="flex items-center justify-between">
                                <p className="font-semibold text-gray-800 text-sm">New Address</p>
                            ...
                            <input value={newAddress.label}
                                ...
                                className={inputClass} placeholder="Label: Home, Office, etc." />
                            <select onChange={e => handleNewAddressCityChange(e.target.value)} className={inputClass}>
                                <option value="">Select City *</option>
                                {cities.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                            </select>
                            <select value={newAddress.barangayName}
                                onChange={e => setNewAddress(prev => ({ ...prev, barangayName: e.target.value }))}
                                disabled={!newAddress.cityName}
                                className={`${inputClass} disabled:bg-gray-50 disabled:text-gray-400`}>
                                <option value="">Select Barangay *</option>
                                {newAddressBarangays.map(b => <option key={b.id} value={b.name}>{b.name}</option>)}
                            </select>
                            <input value={newAddress.streetNo}
                                onChange={e => setNewAddress(prev => ({ ...prev, streetNo: e.target.value }))}
                                className={inputClass} placeholder="Street / House No. (optional)" />
                            <div className="flex gap-3">
                                <button onClick={() => setShowAddAddressForm(false)}
            ...
            {activeTab === 'ids' && (
                <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl text-sm">
                        🪪 Upload valid government IDs for account verification. Required for vehicle purchases.
                    </div>
                    ...
                    <div>
                        ...
                        {primaryIdName ? (
                            <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                                ...
                                <div className="flex-1">
                                    <p className="text-sm font-semibold text-green-700">{primaryIdName}</p>
                                    <p className="text-xs text-green-500">Uploaded successfully (Demo)</p>
                                </div>
                                <button onClick={() => setPrimaryIdName('')}
                            ...
                            <label className="block border-2 border-dashed border-gray-300 rounded-xl p-6 text-center cursor-pointer hover:border-red-400 transition">
                                <p className="text-gray-400 text-sm font-semibold">📄 Click to upload Driver's License</p>
                                <p className="text-gray-300 text-xs mt-1">Front and back • JPG, PNG, PDF • Max 5MB</p>
                                <input type="file" accept="image/*,.pdf" className="hidden"
                                    onChange={e => {
                                        if (e.target.files[0]) {
                                            setPrimaryIdName(e.target.files[0].name);
                                            setSuccess('Primary ID uploaded! (Demo — not saved to server)');
                                            setTimeout(() => setSuccess(''), 3000);
                                        }
                                    }} />
                            </label>
                    ...
                    {/* Secondary ID */}
                    <div>
                        <label className={labelClass}>Secondary ID — PhilSys / TIN / Passport</label>
                        {secondaryIdName ? (
                            <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                                <div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center text-green-600 text-lg">📄</div>
                                <div className="flex-1">
                                    <p className="text-sm font-semibold text-green-700">{secondaryIdName}</p>
                                    <p className="text-xs text-green-500">Uploaded successfully (Demo)</p>
                                </div>
                                <button onClick={() => setSecondaryIdName('')}
                                    className="p-1.5 text-gray-400 hover:text-red-500 transition">
                                    <X size={14} />
                                </button>
                            </div>
                            ...
                            <label className="block border-2 border-dashed border-gray-300 rounded-xl p-6 text-center cursor-pointer hover:border-red-400 transition">
                                <p className="text-gray-400 text-sm font-semibold">📄 Click to upload Secondary ID</p>
                                <p className="text-gray-300 text-xs mt-1">PhilSys, TIN ID, Passport, UMID • Max 5MB</p>
                                <input type="file" accept="image/*,.pdf" className="hidden"
                                    onChange={e => {
                                        if (e.target.files[0]) {
                                            setSecondaryIdName(e.target.files[0].name);
                                            setSuccess('Secondary ID uploaded! (Demo — not saved to server)');
                                            setTimeout(() => setSuccess(''), 3000);
                                        }
                                    }} />
                            </label>
                    ...
                    <div className="bg-amber-50 border border-amber-200 text-amber-700 px-4 py-3 rounded-xl text-xs">
                        ⚠️ For demo purposes only. In production, IDs will be stored securely and reviewed by our Account Management team in compliance with R.A. 10173 (Data Privacy Act of 2012).
                    </div>
                </div>
            )}
            ...
            {activeTab === 'password' && (
                <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    {passwordStep === 'form' ? (
                        <>
                            <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl text-sm">
                                🔒 For security, we'll send an OTP to your email before changing your password.
                            </div>
                            <div>
