...
function MyOrders() {
    ...
    const ORDER_STEPS = ['PENDING','CONFIRMED','PREPARING','READY','OUT_FOR_DELIVERY','DELIVERED','COMPLETED'];
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
                {['ALL','PENDING','CONFIRMED','PREPARING','READY','OUT_FOR_DELIVERY','DELIVERED','COMPLETED','CANCELLED'].map(s => (
                ...
                <div className="space-y-3">
                    {filtered.map(o => (
                        <div key={o.id}
                            ...
                            onClick={() => { setSelected(o); setShowModal(true); setError(''); }}>
                            <div className="flex items-start justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400">{o.vehicleYear} • {o.vehicleColor} • Order #{o.id}</p>
                                        <p className="text-red-600 font-bold text-sm mt-1">₱{Number(o.amount).toLocaleString()}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">{new Date(o.createdAt).toLocaleDateString('en-PH')}</p>
                                </div>
                            </div>
                                ...
                                <div className="mt-4">
                                    <div className="w-full bg-gray-100 rounded-full h-1.5">
                                        <div className="bg-red-500 h-1.5 rounded-full transition-all duration-500"
                                            style={{ width: `${Math.max(5, ((getStepIndex(o.status) + 1) / ORDER_STEPS.length) * 100)}%` }} />
                                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    { label: 'Vehicle', value: `${selected.vehicleBrand || '—'} ${selected.vehicleModel || ''}`.trim() },
                            ...
                            {selected.receiptGenerated && (
                                <button onClick={async (e) => {
                                    ...
                                    const res = await fetch(`${BASE_URL}/api/buyer/orders/${selected.id}/receipt`, {
