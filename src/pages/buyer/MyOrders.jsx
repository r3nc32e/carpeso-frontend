            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-bold ${statusColor(o.status)}`}
                  >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-center">
                <span
                  className={`px-4 py-2 rounded-full text-sm font-bold ${statusColor(selected.status)}`}
                >
                  ...
                  <div className="space-y-2">
                    {ORDER_STEPS.map((step, i) => (
                      <div key={step} className="flex items-center gap-3">
                        ...
                        <span
                          className={`text-sm font-semibold ${i <= getStepIndex(selected.status) ? "text-gray-800" : "text-gray-300"}`}
                        >
                          {step.replace(/_/g, " ")}
                        </span>
                      </div>
                ...
                {[
                  {
                    label: "Vehicle",
                    value:
                      `${selected.vehicleBrand || "—"} ${selected.vehicleModel || ""}`.trim(),
                  },
                  { label: "Year", value: selected.vehicleYear || "—" },
                  { label: "Color", value: selected.vehicleColor || "—" },
                  {
                    label: "Amount",
                    value: `₱${Number(selected.amount || 0).toLocaleString()}`,
                  },
                  {
                    label: "Payment Mode",
              ...
              {selected.warrantyStartDate && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                  ...
                  <p className="text-sm text-green-700">
                    {new Date(selected.warrantyStartDate).toLocaleDateString(
                      "en-PH",
                    )}{" "}
                    —{" "}
                    {selected.warrantyEndDate
                      ? new Date(selected.warrantyEndDate).toLocaleDateString(
                          "en-PH",
                        )
                      : "N/A"}
                  </p>
                ...
                <button
                  onClick={async (e) => {
                    ...
                    const res = await fetch(
                      `http://localhost:8080/api/buyer/orders/${selected.id}/receipt`,
                      {
