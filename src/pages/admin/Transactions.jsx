          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div>
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-bold ${statusColor(selected.status)}`}
                >
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      {
                        label: "Vehicle",
                        value: `${selected.vehicleBrand} ${selected.vehicleModel}`,
                      },
                      { label: "Year", value: selected.vehicleYear || "—" },
                      { label: "Color", value: selected.vehicleColor || "—" },
                      {
                        label: "Amount",
                        value: `₱${Number(selected.amount).toLocaleString()}`,
                      },
                      {
                        label: "Payment Mode",
                        value: selected.paymentMode?.replace(/_/g, " ") || "—",
                      },
                      {
                        label: "Order Date",
                      ...
                      },
                      {
                        ...
                        value: selected.expiresAt
                          ? new Date(selected.expiresAt).toLocaleString("en-PH")
                          : "—",
                      },
                      {
                        label: "Receipt No.",
                        value: selected.receiptNumber || "Not yet generated",
                      },
                    ].map(({ label, value }) => (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      {
                        label: "Barangay",
                        value: selected.buyerBarangayName || "—",
                      },
                      ...
                      <div className="space-y-3">
                        {selected.buyerPrimaryIdUrl && (
                          <div>
                            ...
                            <div className="border border-gray-200 rounded-xl overflow-hidden">
                              <img
                                src={`${IMG_BASE}${selected.buyerPrimaryIdUrl.replace("/uploads", "")}`}
                                alt="Primary ID"
                            ...
                            <a
                              href={`${IMG_BASE}${selected.buyerPrimaryIdUrl.replace("/uploads", "")}`}
                              target="_blank"
                        ...
                        {selected.buyerSecondaryIdUrl && (
                          <div>
                            ...
                            <div className="border border-gray-200 rounded-xl overflow-hidden">
                              <img
                                src={`${IMG_BASE}${selected.buyerSecondaryIdUrl.replace("/uploads", "")}`}
                                alt="Secondary ID"
                                className="w-full h-32 object-contain bg-gray-50"
                                onError={(e) => {
                                  e.target.style.display = "none";
                            ...
                            <a
                              href={`${IMG_BASE}${selected.buyerSecondaryIdUrl.replace("/uploads", "")}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-xs text-blue-600 hover:underline mt-1 block"
                            >
                      ...
                      {[
                        ...
                        return (
                          <div key={step} className="flex items-center gap-3">
                            <div
                              className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${stepIdx < currentIdx ? "bg-green-500 text-white" : stepIdx === currentIdx ? "bg-red-600 text-white" : "bg-gray-200 text-gray-400"}`}
                            >
                              {stepIdx < currentIdx ? "✓" : i + 1}
                            </div>
                            <span
                              className={`text-xs font-semibold ${stepIdx <= currentIdx ? "text-gray-800" : "text-gray-300"}`}
                            >
                              {step.replace(/_/g, " ")}
                            </span>
                          </div>
                ...
                <div className="border-t border-gray-100 pt-4 space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {NEXT_STATUS[selected.status] && (
                      <button
                        onClick={() =>
                          setNewStatus(NEXT_STATUS[selected.status])
                        }
              ...
              {isFinal(selected.status) && (
                <div
                  className={`px-4 py-3 rounded-xl text-sm font-semibold text-center ${selected.status === "COMPLETED" ? "bg-green-50 text-green-700" : selected.status === "CANCELLED" ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-500"}`}
                >
