...
function Overview() {
  ...
  return (
    <div className="space-y-6">
          ...
          {visibleCards.map((card) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div
                  className={`w-12 h-12 ${card.color} rounded-xl flex items-center justify-center text-white group-hover:scale-110 transition`}
                >
                  {card.icon}
                </div>
              </div>
      ...
      {canAccess("SALES_ANALYST") && stats?.totalRevenue != null && (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    {["#", "Buyer", "Vehicle", "Amount", "Status", "Date"].map(
                      (h) => (
                        <th
                          key={h}
                          className="text-left py-3 px-4 text-xs font-bold text-gray-400 uppercase"
                        >
                          {h}
                        </th>
                      ),
                ...
                <tbody>
                  {recentTransactions.map((t) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 font-semibold text-gray-800 text-xs">
                        {t.buyerFullName}
                      </td>
      ...
      {!isSuperAdmin && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
          ...
          <p className="text-xs text-blue-500">
            You can only access pages and data assigned to your role. Contact
            the SuperAdmin to request additional access.
          </p>
        </div>
      )}
    </div>
